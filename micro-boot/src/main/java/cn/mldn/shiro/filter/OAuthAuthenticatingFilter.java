...
import cn.mldn.shiro.token.OAuthToken;
...
public class OAuthAuthenticatingFilter extends AuthenticatingFilter {
	...
	protected boolean onAccessDenied(ServletRequest request, ServletResponse response) throws Exception {
		...
		if (!(error == null || "".equals(error))) {	// 现在出现有错误提示信息
			...
			return false ; // 后续的操作不再执行，而直接斤西瓜跳转处理
		}
	...
	protected boolean onLoginFailure(AuthenticationToken token, AuthenticationException e, ServletRequest request,
		...
		if (subject.isAuthenticated() || subject.isRemembered()) {	// 失败与你无关
			try { // 已经登录成功了就返回到首页上
